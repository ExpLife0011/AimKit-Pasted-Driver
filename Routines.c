...
PVOID pSource = NULL, pTarget = NULL, fTarget = NULL;
...
//if (MmIsAddressValid(fTarget))
//{
__try
{
ProbeForRead(fTarget, pCopy->size, 1);
}
...
DPRINT( "AKDriver: %s: PsLookupProcessByProcessId failed with status 0x%X\n", __FUNCTION__, status );
...
NTSTATUS BBCopyMemory( IN PCOPY_MEMORY pCopy )
{
	...
	//	DPRINT("BlackBone: pid:%d src: 0x%llx trg: 0x%llx size %d\n", pCopy->pid, pSource, pTarget, pCopy->size);
        status = MmCopyVirtualMemory( pSourceProc, pSource, pTargetProc, pTarget, pCopy->size, UserMode, &bytes );
    }
    else
        DPRINT( "BlackBone: %s: PsLookupProcessByProcessId failed with status 0x%X\n", __FUNCTION__, status );
...
{
	////VMProtectBeginUltra("BBAllocateFreePhysical");
    NTSTATUS status = STATUS_SUCCESS;
...
{
	//VMProtectBeginUltra("BBProtectMemory");
    NTSTATUS status = STATUS_SUCCESS;
    PEPROCESS pProcess = NULL;
