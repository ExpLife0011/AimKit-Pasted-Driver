    ...
    )
{
    ...
    if (NT_SUCCESS( status ))
    {
        ...
        RtlUnicodeStringInit( &ustrNtdll, VMProtectDecryptStringW(L"ntdll.dll") );

        PVOID pNtdll = BBGetUserModule( context.pProcess, &ustrNtdll, PsGetProcessWow64Process( pProcess ) != NULL );

        context.pSetEvent = BBGetModuleExport( pNtdll, VMProtectDecryptStringA("NtSetEvent"), pProcess, NULL );
        context.pLoadImage = BBGetModuleExport( pNtdll, VMProtectDecryptStringA("LdrLoadDll"), pProcess, NULL );
    }
...
{
	VMProtectBeginUltra("BBFindOrMapModule");
    NTSTATUS status = STATUS_SUCCESS;
    ...
    if (!(flags & KNoSxS) && NT_SUCCESS( status ))
    {
        ...
        if (pManifest && manifestID != 0)
        {
            ...
            if (buffer != NULL)
            {
                ...
                RtlInitUnicodeString( &manifestPathNt, VMProtectDecryptStringW(L"\\SystemRoot\\Temp\\BBImage.manifest") );
                ...
                {
                    // Cast path
                    RtlUnicodeStringPrintf( &manifestPath, L"%ls\\%ls", SharedUserData->NtSystemRoot, VMProtectDecryptStringW(L"\\Temp\\BBImage.manifest") );
    ...
    )
{
    ...
    PVOID pQueryName = BBGetModuleExport( hNtdll, VMProtectDecryptStringA("RtlDosApplyFileIsolationRedirection_Ustr"), pContext->pProcess, NULL );
    ...
    )
{
    ...
    if (NT_SUCCESS( BBResolveApiSet( pProcess, &filename, baseImage, resolved ) ))
    {
        ...
        if (PsGetProcessWow64Process( pProcess ) != NULL)
            RtlUnicodeStringCatString( &fullResolved, VMProtectDecryptStringW(L"\\SystemRoot\\syswow64\\") );
        else
            RtlUnicodeStringCatString( &fullResolved, VMProtectDecryptStringW(L"\\SystemRoot\\system32\\") );
